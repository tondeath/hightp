...
The most important problems here was that we count sites to be overlap even when there is just 1 bp overlap. This might result in overestimation the degree of association between CpG island and promoter (how much are meaningful overlap are), as there might be no actual biological interaction between CpG island and promoter that overlap by just 1 bp. 
...
From the both plot, we can see that mean intensity of signal increased as the positions became closer to position 0, but the peak positions were different. For ChIP, the peak intensity is at position 0, while for the CAGE the peak intensity is at around 100 bp downstream of position 0 (around -100 on minus strand, and at around +100 on plus strand). 

What we see here is important as it actually told us about the relative position of TSS and enhancer region that p300 bind to. From the results, TSS is mostly located approximately downstream of enhancer region on both strands and especially found 100 bp downstream, but TSS can also be found around 100 bp upstream and 400 bp downstream of center of enhancer. It also told us that p300 mostly bind upstream of TSS at center of enhancer. These results also agree with and confirm our knowledeg about typical gene structure that TSS is located downstream of enhancer, but it also told us that there is some enhancer that locate downstream of TSS as far as 100 bp downstream.
