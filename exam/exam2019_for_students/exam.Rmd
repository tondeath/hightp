...
contig_table <- as.tibble(contig_mat, rownames = NA) %>% mutate(proportion.single = single.exon/rowSums(.))
...
```{r}
fisher.test(contig_mat[,1:2]) # use 2 side bc it can be both situation and both are intersting unlike enrichment analysis below in 3rd question
```
