...
We read studyDesign.tsv and diabetesGenes.tsv file using read_tsv(), but we need to read countMatrix.tsv and normalizedMatrix.tsv using read.table as read_tsv could not handle file which the number of column name in header not equal to number of column (e.g. the file with row names). Then we printed out head rows of each tibble or data frame to check the integrity. We can see that all files were read in correctly, and the number and name of samples in design tibble were the same as in other data frame.
...
From the result, we can see that the fitted trend line curved downward and did not follow the expected trend line. We can see that the distribution of point on MA-plot was not symmetric. The highly expressed genes were downregulated more than expected. This indicated the problem in normalization step during DESeq analysis. We can also see some data point that adjusted p-value cannot be caluculated due to problem with read count e.g. zero counts or extreme count outlier [1].
...
Contigency table obtained from question 3.3.2 was converted to 2x2 matrix with row and column name, so that we can easily convert it further to 2x2 tibble. We than calculated the proportion of diferentially expressed genes in groups of diabetes and non-diabetes genes. The resulting tibble was converted back to matrix so that is can be input to fisher.test() function. The contigency table for Fisher, exact test was as below.

Here, we wanted to know whether the proportion of diabetes gene was significantly larger in differentialy expressed genes (was enriched) than in the genes that were not differentially expressed or not. Therefore, we use one-sided Fisher's exact test to test the different in proportion with alpha = 0.05. We chose one-side test, beacuse here we only interest in testing whether the proportion of diabetes genes in DE genes is larger than in non-DE genes or not. So that we can say that diabetes genes were overepresented in case that the proportion is significantly larger. We did not interest in the case where that proportion in non-DE genes was larger than in DE genes.
...
From the contigency table above, we can see that the proportion of diabetes genes in group of diferentially expressed genes is greater than the proportion of diabetes genes in the group of non diferentially expressed genes. However, the diffrence is very small (very small effective size). 

The one-side Fisher's exact test results confirm that this difference is not significant. We can see that the p-value from the test is 0.497 which is much larger than our alpha = 0.05. So, we retain null hypothesis that the proportion of diabetes genes in the group of diferentially expressed genes was not different from the proportion of diabetes genes in the group of non diferentially expressed genes. Therefore, the Novo Nordisk's claims that the treatment affects expression of diabetes-related genes is not true here. 
...
1.https://bioconductor.org/packages/release/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#pvaluesNA
